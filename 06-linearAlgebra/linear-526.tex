...
spaces. Also, some theorems and proofs become shorter and more
elegant. 
...
subsets of a vector space (without a norm). For normed vector spaces,
these definitions are the same as the definitions using open and
closed sets.
\begin{definition}
...
point $v$ and remain inside $A$.
\begin{definition}
  Let $V$ be a vector space and $A \subseteq V$. The \textbf{algebraic
    closure}\footnote{This is non-standard terminology.} of $A$ is set
...
\begin{proof}[Proof of theorem \ref{thm:sht}]
  ...
  impossible. TODO: make a lemma proving this. Regardless of the
