...
Let $A$ be an $m$ by $n$ matrix and $b \in \R^m$ either:

1. $\exists x = (x_1, ..., x_n) \in \R^n$ such that $x_1
   \geq 0, ..., x_n \geq 0$ and $Ax = b$ or
2. $\exists y \in \R^m$ such that $y^T A e_i \geq 0$ for
...
> - Local non-satiation : for each $x$ and $\epsilon>0$ $\exists x'$
  such that $\norm{x-x'} \leq \epsilon$ and $x' \succ_i x$
