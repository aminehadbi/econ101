...
\begin{definition}[Relation]
  ...
  $a \sim_R b$ if $(a,b) \in R$ (where $\sim_R$ could be some other symbol).
\end{definition}
Usually, $A=B$, and then we say $\sim_R$ is a relation on $A$. 
\begin{example}
...
\begin{definition}[Properties of relations]  
  A relation $\sim_R$ on $A$ is 
  \begin{itemize}
  \item \textbf{reflexive} if $a \sim_R a$ $\forall a \in A$,
  \item \textbf{symmetric} if $a \sim_R b$ implies $b \sim_R a$,
  \item \textbf{transitive} if $a \sim_R b$ and $b \sim_R c$ implies
    $a \sim_R c$,
  \item \textbf{antisymmetric} if $a \sim_R b$ and $b \sim_R a$
    implies $a=b$,
  \item \textbf{asymmetric} if $a \sim_R b$ implies $b$ is not
    $\sim_R a$, and
  \item \textbf{complete} if either $a \sim_R b$ or $b \sim_R a$ or
