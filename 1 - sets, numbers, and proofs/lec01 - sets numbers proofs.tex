...
$2^{\aleph_0}$ or $\mathbb{c}$. You might be wondering whether there
...
1900, Hilbert made a famous list of 23 unsolved important problemts in
...
operations. $(\mathbb{Q},+,\cdot)$ is a field. Other fields that you
may have encountered include the complex numbers with the usual
addition and multiplication, and the intergers modulo any prime
number.
...
the rational numbers. An \textbf{ordered set} is a set, $A$, and a
relation, $<$, such that (i) $\forall a,b \in A$ either $a < b$ or $a
= b$ or $a > b$; and (ii) if $a < b$ and $b < c$ then $a < c$. An
\textbf{ordered field} is a field that is an ordered set and addition
and multiplication preserve the ordering in that (i) if $b<c$ then $a
+ b < a + c$ (ii) if $a>0$ and $b>0$ then $ab>0$.
