...
\begin{exercise}
  ...
  properties $=$, $<$, and $\leq$ on $\Re$ have.
\end{exercise}
Slightly confusingly, symmetric and antisymmetric are not
opposites. The usual equality in $\Re$, $=$ is both symmetric and
antisymmetric.
...
\begin{example}[Isoquants]
  Consider a production function $f: \Re^n \to \Re$. We can define an
  equivalence relation on $\Re^n$ by $x \sim z$ iff $f(x) = f(z)$. The
...
The usual order on $\Re$, $\leq$ has two additional properties that a
generic order need not have. First, any set in $\Re$ with an upper
bound has a least upper bound. $y$ is an upper bound of $ A \subseteq
\Re$, if $y \geq x$ for all $x \in A$. $y$ is the least upper bound of
$A$ if there is no $z < y$ that is also an upper bound of $A$. Least
upper bounds in $\Re$ are unique. Among other things, uniqueness of
...
\end{example}
\begin{example}
  A partial order on $\Re^n$ can be defined by $x \leq y$ if $x_i \leq
  y_i$ for all $i$.
\end{example}
A preference relation is not a partial order because it allows
indifference between goods that are not the same. 

Another aspect of the usual $\leq$ order on $\Re$ is that all elements
...
manner. To precisely state Arrow's result, we first need one more
definition. Suppose we have a way to construct a social order $\succ$ from
the individual preferences $\pref_i$. This construction satisfies the
\textbf{independence of irrelevant alternatives} if for every $A
\subseteq X$, if $\pref_i$ and $\pref_i'$ are two sets of individuals
preferences that agree on $A$, 
\[ x \pref_i y \iff x \pref_i' y \; \forall x,y \in A \] 
then $\succ$ and $\succ'$ also agree on $A$. Independence of irrelevant
alternatives says that the question of whether society likes apples
better than bananas should not depend on what anyone thinks about the
comparison between apples and oranges (or oranges and kiwis, etc). It
is an intuitively desirable feature of a social ordering.

One social ordering that completes the Pareto order is the dictatorial
ordering. That is, we let any one person's preference be society's
preference. 
