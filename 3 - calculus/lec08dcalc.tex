...
\begin{proof}
  ...
  for any $\epsilon>0$. 
  Combined with an earlier equation now we have,
  \begin{align} 
...
open set around $x$ for the function to be differentiable at
$x$. Nonetheless, the following is true:
\begin{lemma}\label{lem:gatfre}
  If $f$ has continuous G\^{a}teaux derivatives in an open
  neighborhood, $N_\delta(x_0)$, and moreover, 
  \[  \lim_{\alpha \to 0} \sup_{x \in N_\delta(x_0), v \in V}  \norm{ \frac{f(x + \alpha v) - f(x)
      - df(x;v)\alpha}{\alpha}} = 0 \]
  then $Df_{x_0}$ exists and $Df_{x_0}(v) = df(x_0;v)$
\end{lemma}
\begin{remark}
  This sort of $\lim \sup$ requirement is called uniform convergence
  in general, and uniform differentiability in this particular
  case. It was not required in finite dimensions because we could look
  at basis directions and make things less than $\epsilon/n$. In the
  infinite dimensional case, this does not work because
  $\epsilon/\infty=0$.
\end{remark}
\begin{remark}
  I have not written out the proof of this theorem, or checked its
  statement against a reliable source. I am sure that the idea:
  uniform G\^{a}teaux differentiable implies Fr\'{e}chet
  differentiable, but you should probably double check the lemma
  before relying on all its details.
\end{remark}
\begin{example*}[Example \ref{ex:fre} continued]
  ...
  \begin{align*}
    ...
    d\tau
  \end{align*}    
  Now, we can either check that these derivatives are uniform, or just
