...
compact sets are always closed. In $\R^n$, compactness is equivalent
to being closed and bounded, two properties that are easier to
understand.
\begin{definition}
...
bounded sets that are not compact.
\begin{example}
...
if every sequence in $K$ has a subsequence that converges in $K$. In
$\R^n$, a set is sequentially compact iff it is compact. 
...
true. We saw above that in infinite dimensional normed vector spaces,
there are closed and bounded sets which are not compact. 
However it is always true that sequential compactness and
...
compact. In any metric space, a set is compact iff it is sequentially
compact. 
