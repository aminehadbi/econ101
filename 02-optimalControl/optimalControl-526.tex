...
\end{align}
Just as when maximizing with respect to vectors, the set of $v$ and
...
\begin{example}
  ...
  \begin{align*}
    x(t) = & \frac{p(T-t) - s}{2c} \\
    y(t) = & y(0) + t\frac{pT-s}{2c} - \frac{p}{4c}t^2
  \end{align*}
\end{example}
...
\begin{example}[Inventory] % FIXME: check algebra
  ...
  \begin{align*}
    ...
    y_T = & \frac{s(T^2 - \bar{t}^2) + 2\lambda(0) (T-\bar{t})}{4c} \\
    \lambda(0) = & y_T \frac{2c}{T-\bar{t}} - \frac{s}{2}(T+\bar{t}).
  \end{align*}
...
Transversality conditions for infinite horizon problems are somewhat
...
\begin{example}[Non-standard transversality conditions \label{ex:tc}]
...
The first order condition for $q$ is that for all $x$,
\begin{align*}
